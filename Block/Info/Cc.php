...
class Cc extends \Magento\Payment\Block\Info\Cc
{
    ...
    protected function _prepareSpecificInformation($transport = null)
    {
        ...
        if (($transactionId = $info->getAdditionalInformation(Shift4Method::KEY_SHIFT4_TRANSACTION_ID))) {
            $data[(string)__('Transaction Id (RRN)')] = $transactionId;
        }
        if (($trxid = $info->getAdditionalInformation(Shift4Method::KEY_SHIFT4_3DS_TRXID))) {
            $data[(string)__('3DS transaction ID')] = $trxid;
        }
        if ($this->appState->getAreaCode() === \Magento\Framework\App\Area::AREA_ADMINHTML) {
            if (($eci = $info->getAdditionalInformation(Shift4Method::KEY_SHIFT4_3DS_VERSION))) {
                $data[(string)__('3DS version')] = $eci;
            }
            if (($eci = $info->getAdditionalInformation(Shift4Method::KEY_SHIFT4_3DS_ECI))) {
                $data[(string)__('3DS ECI')] = $eci;
            }
            if (($authCode = $info->getAdditionalInformation(Shift4Method::KEY_SHIFT4_AUTH_CODE))) {
                $data[(string)__('Auth code')] = $authCode;
            }
            if (($responseId = $info->getAdditionalInformation(Shift4Method::KEY_SHIFT4_RESPONSE_ID))) {
                $data[(string)__('Response ID')] = $responseId;
            }
            if (($riskScore = $info->getAdditionalInformation(Shift4Method::KEY_SHIFT4_RISK_SCORE))) {
